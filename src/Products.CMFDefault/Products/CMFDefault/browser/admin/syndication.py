...
class Site(SettingsEditFormBase):
    ...
    label = _("Configure Portal Syndication")

    actions = form.Actions(
        form.Action(
            name="enable",
            label=_("Enable Syndication"),
            condition="disabled",
    ...
    def handle_enable(self, action, data):
        ...
        self.status = _("Syndication enabled.")
        self._setRedirect(*self.redirect)

    def handle_change(self, action, data):
        self._handle_success(action, data)
        self.status = _("Syndication settings changed.")
        self._setRedirect(*self.redirect)

    def handle_disable(self, action, data):
        self.getContent().disable()
        self.status = _("Syndication disabled.")
        self._setRedirect(*self.redirect)
...
class Folder(Site):
    ...
    label = _("Configure Folder Syndication")

    actions = form.Actions(*Site.actions.actions)
    actions.append(
        form.Action(
        name="revert",
        label=_("Revert to Site Default"),
        condition="enabled",
    ...
    def handle_revert(self, action, data):
        ...
        self.status = _("Syndication reset to site default.")
        self._setRedirect(*self.redirect)
