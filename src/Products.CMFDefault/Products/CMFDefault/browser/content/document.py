...
available_text_formats = (
        ('structured-text', 'structured-text', _('structured-text')),
...
class DocumentEditView(ContentEditFormBase):
    ...
    def handle_validate(self, action, data):
        ...
        if not self.context.isValidSafetyBelt(safety_belt):
            return (_('Intervening changes from elsewhere detected. Please '
                      'refetch the document and reapply your changes.'),)
        return errors
