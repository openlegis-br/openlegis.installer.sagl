...
class IFavoriteSchema(Interface):
    ...
    remote_url = BytesLine(
        title=_('URL'),
        description=_('URL relative to the site root.'),
        required=False,
        missing_value='')
...
class FavoriteURIWidget(BytesWidget):
    ...
    def _toFieldValue(self, input):
        ...
        tokens = urllib.parse.urlparse(value, 'http')
        if tokens[1]:
            # There is a nethost, remove it
            tokens = ('', '') + tokens[2:]
            value = urllib.parse.urlunparse(tokens)
...
class FavoriteAddView(ContentAddFormBase):
    ...
    form_fields = (
        form.FormFields(ASCIILine(__name__='id', title=_('ID'))) +
