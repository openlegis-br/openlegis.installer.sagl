...
def contents_delta_vocabulary(context):
    ...
    deltas = [SimpleTerm(i, str(i), str(i))
            for i in list(range(1, min(5, length))) + list(range(5, length, 5))]
...
class ContentsView(BatchFormMixin, EditFormBase):
    ...
    @property
    def label(self):
        return _('Folder Contents: ${obj_title}',
    ...
    def column_headings(self):
        ...
        for column in columns:
            ...
            query = urllib.parse.urlencode(paras)
    ...
    def validate_items(self, action=None, data=None):
        ...
        if errors:
            return errors
        if self._get_ids() == []:
            errors.append(_("Please select one or more items first."))
        return errors
    ...
    def handle_cut_success(self, action, data):
        ...
        try:
            ...
            if len(ids) == 1:
                self.status = _('Item cut.')
            else:
                self.status = _('Items cut.')
        except CopyError:
            self.status = _('CopyError: Cut failed.')
            return self.handle_failure(action, data, ())
        except zExceptions_Unauthorized:
            return self.handle_failure(action, data, ())
            self.status = _('Unauthorized: Cut failed.')
    ...
    def handle_copy_success(self, action, data):
        ...
        try:
            ...
            if len(ids) == 1:
                self.status = _('Item copied.')
            else:
                self.status = _('Items copied.')
        except CopyError:
            self.status = _('CopyError: Copy failed.')
            return self.handle_failure(action, data, ())
        return self._setRedirect('portal_types', 'object/folderContents')
    ...
    def handle_paste_success(self, action, data):
        ...
        try:
            ...
            if len(result) == 1:
                self.status = _('Item pasted.')
            else:
                self.status = _('Items pasted.')
        except CopyError:
            self.request.response.expireCookie('__cp',
                    path='%s' % (self.request['BASEPATH1'] or "/"))
            self.status = _('CopyError: Paste failed.')
        except ValueError:
            self.status = _('ValueError: Paste failed.')
            return self.handle_failure(action, data, ())
        except zExceptions_Unauthorized:
            self.status = _('Unauthorized: Paste failed.')
            return self.handle_failure(action, data, ())
        return self._setRedirect('portal_types', 'object/folderContents')
    ...
    def handle_delete_success(self, action, data):
        ...
        if len(ids) == 1:
            self.status = _('Item deleted.')
        else:
            self.status = _('Items deleted.')
        return self._setRedirect('portal_types', 'object/folderContents')
    ...
    def handle_up_success(self, action, data):
        ...
        try:
            ...
            if attempt == 1:
                self.status = _('Item moved up.')
            elif attempt > 1:
                self.status = _('Items moved up.')
            else:
                self.status = self.noChangesMessage
        except ValueError:
            self.status = _('ValueError: Move failed.')
            return self.handle_failure(action, data, ())
        return self._setRedirect('portal_types', 'object/folderContents')
    ...
    def handle_down_success(self, action, data):
        ...
        try:
            attempt = self.context.moveObjectsDown(ids, delta,
                                                 subset_ids=subset_ids)
            if attempt == 1:
                self.status = _('Item moved down.')
            elif attempt > 1:
                self.status = _('Items moved down.')
            else:
                self.status = self.noChangesMessage
        except ValueError:
            self.status = _('ValueError: Move failed.')
    ...
    def handle_top_success(self, action, data):
        ...
        try:
            attempt = self.context.moveObjectsToTop(ids,
                                                    subset_ids=subset_ids)
            if attempt == 1:
                self.status = _('Item moved to top.')
            elif attempt > 1:
                self.status = _('Items moved to top.')
            else:
                self.status = self.noChangesMessage
        except ValueError:
            self.status = _('ValueError: Move failed.')
    ...
    def handle_bottom_success(self, action, data):
        ...
        try:
            attempt = self.context.moveObjectsToBottom(ids,
                                                       subset_ids=subset_ids)
            if attempt == 1:
                self.status = _('Item moved to bottom.')
            elif attempt > 1:
                self.status = _('Items moved to bottom.')
            else:
                self.status = self.noChangesMessage
        except ValueError:
            self.status = _('ValueError: Move failed.')
    ...
    def handle_sort_order_success(self, action, data):
        ...
        self.status = _('Default sort order changed.')
        return self._setRedirect('portal_types', 'object/folderContents')
    ...
    def handle_rename_success(self, action, data):
        ...
        if not ids == new_ids:
            try:
                ...
                if len(ids) == 1:
                    self.status = _('Item renamed.')
                else:
                    self.status = _('Items renamed.')
            except CopyError:
                self.status = _('CopyError: Rename failed.')
...
class FolderShareView(EditFormBase):
    ...
    def __call__(self, ADD=None, DELETE=None):
        if ADD:
            ...
            return self.index()
        self.status = _('Local Roles changed.')
