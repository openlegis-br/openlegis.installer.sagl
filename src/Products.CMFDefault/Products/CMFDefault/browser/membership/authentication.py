...
"""
try:
   from urllib.parse import quote #py2
...
class ILoginSchema(Interface):
    ...
    persistent = Bool(
        title=_('Remember my ID.'),
        description=_('Saves your member ID in a cookie.'),
        default=True)
...
class IMailPasswordSchema(Interface):
    ...
    name = Choice(
        title=_('Member ID'),
        description=_('Member ID or email address'),
...
class MailPasswordFormView(EditFormBase):
    ...
    label = _("Don't panic!")
    description = _("Just enter your member ID below, click 'Send', and "
                    "your password will be mailed to you if you gave a "
                    "valid email address when you signed on.")
    ...
    def handle_send_success(self, action, data):
        ...
        self.status = _('Your password has been mailed to you.')
