...
class IJoinSchema(Interface):
    ...
    send_password = Bool(
        title=_("Mail Password?"),
        description=_("Check this box to have the password mailed."))

    @invariant
    def check_passwords_match(schema):
        """Password and confirmation must match"""
        if schema.password != schema.confirmation:
            raise Invalid(_("Your password and confirmation did not match. "
                            "Please try again."))
...
class JoinFormView(EditFormBase):
    ...
    def label(self):
        if self.isManager:
            return _('Register a New Member')
        else:
            return _('Become a Member')
    ...
    def handle_register_validate(self, action, data):
        ...
        if not rtool.isMemberIdAllowed(data['member_id']):
            errors.append(_("The login name you selected is already in use "
...
#            return self.handle_failure(action, data, ())
        ...
        if self.isManager:
            if self._notify_member(data):
                self.status = _('Member registered and notified.')
            else:
                self.status = _('Member registered.')
            return self._setRedirect('portal_actions',
                                     'global/members_register',
                                     keys='b_start')
        if self._notify_member(data):
            self.status = _('You will receive an email shortly containing '
                            'your password and instructions on how to '
                            'activate your membership.')
        else:
            self.status = _('You have been registered as a member.')
        try:
