...
class Manage(BatchFormMixin, EditFormBase):
    ...
    _NEXT_PLURAL_MESSAGE = _('Next ${count} members')
    _NEXT_SINGULAR_MESSAGE = _('Next member')
    _PREV_PLURAL_MESSAGE = _('Previous ${count} members')
    _PREV_SINGULAR_MESSAGE = _('Previous member')
    ...
    def description(self):
        if not self.members_exist():
            return _('Currently there are no members registered.')
        return ''
    ...
    def validate_items(self, action=None, data=None):
        ...
        if errors:
            return errors
        if self._get_ids() == []:
            errors.append(_("Please select one or more members first."))
        return errors
    ...
    def handle_delete(self, action, data):
        ...
        self.status = _("Selected members deleted.")
    ...
    def listBatchItems(self):
        ...
        for member in self._getBatchObj():
            member_home = self.mtool.getHomeUrl(member['id'],
                                                verifyPermission=1)
            member_listed = member['listed'] and _('Yes') or _('No')
