...
class IPreferencesSchema(Interface):
    ...
    fullname = TextLine(
        title=_('Full Name'),
        description=_('given names and surname'),
        required=False,
        missing_value='')

    email = EmailLine(
        title=_('Email Address'),
        description=_('info@example.org'))

    listed = Bool(
        title=_('Listed status'),
        description=_("Select to be listed on the public membership roster."))

    portal_skin = Choice(
        title=_('Skin'),
        vocabulary="cmf.AvailableSkins",
        required=False,
        missing_value='')
...
class PreferencesFormView(SettingsEditFormBase):
    ...
    label = _("Member Preferences")
    successMessage = _("Member preferences changed.")
    ...
    def applyChanges(self, data):
        ...
        if any('portal_skin' in v for v in changes.values()):
            stool = queryUtility(ISkinsTool)
