...
class BatchViewBase(ViewBase):
    ...
    _BATCH_SIZE = 25
    _NEXT_PLURAL_MESSAGE = _('Next ${count} items')
    _NEXT_SINGULAR_MESSAGE = _('Next item')
    _PREV_PLURAL_MESSAGE = _('Previous ${count} items')
    _PREV_SINGULAR_MESSAGE = _('Previous item')
    ...
    def _getNavigationURL(self, b_start):
        ...
        return '{0}{1}'.format(target, query)
    ...
    def navigation_next(self):
        batch_obj = self._getBatchObj().__next__
        if batch_obj is None:
            return None
    ...
    def summary_type(self):
        length = self._getBatchObj().sequence_length
        return (length == 1) and _('item') or _('items')
...
class BatchFormMixin(BatchViewBase):
    ...
    def _getNavigationURL(self, b_start):
        ...
        query = kw and ('?%s' % urllib.parse.urlencode(kw)) or ''
        return '%s%s' % (target, query)
