...
class _EditFormMixin(ViewBase):
    ...
    noChangesMessage = _('Nothing to change.')
...
class SettingsEditFormBase(_EditFormMixin, form.PageForm):
    ...
    description = ''
    successMessage = _("Settings changed.")
...
class ContentAddFormBase(_EditFormMixin, form.PageAddForm):
    ...
    def label(self):
        ...
        return _('Add ${obj_type}', mapping={'obj_type': obj_type})
    ...
    def add(self, obj):
        ...
        self.status = _('${obj_type} added.', mapping={'obj_type': obj_type})
...
class FallbackAddView(ContentAddFormBase):
    ...
    form_fields = form.FormFields(ASCIILine(__name__='id', title=_('ID')))
...
class ContentEditFormBase(SettingsEditFormBase):
    ...
    def label(self):
        ...
        return _('Edit ${obj_type}', mapping={'obj_type': obj_type})

    @property
    def successMessage(self):
        obj_type = translate(self.context.Type(), self.context)
        return _('${obj_type} changed.', mapping={'obj_type': obj_type})

    def handle_validate(self, action, data):
        if self.context.wl_isLocked():
            return (_('This resource is locked via webDAV.'),)
