...
created.append({'value': '1970/01/01 00:00:01 GMT', 'title': _('Ever')})
if not mtool.isAnonymousUser():
    member = mtool.getAuthenticatedMember()
    created.append({'value': member.getProperty('last_login_time'),
                    'title': _('Last login')})
created.append({'value': (today-1).Date(), 'title': _('Yesterday')})
created.append({'value': (today-7).Date(), 'title': _('Last week')})
created.append({'value': (today-31).Date(), 'title': _('Last month')})
...
buttons.append( {'name': 'search', 'value': _('Search')} )
