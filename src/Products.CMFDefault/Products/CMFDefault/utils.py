...
def parseHeadersBody( body, headers=None, rc=re.compile( r'\n|\r\n' ) ):
    ...
    """
    buffer = BytesIO(body)
    message = email.rfc822.Message(buffer)
        ...
        if tag in self.valid_tags:
            ...
            for k, v in attrs:
                ...
                if k.lower().startswith('on'):
                    msg = _("JavaScript event '${attribute}' not allowed.",
                            mapping={'attribute': k})
                    raise IllegalHTML(msg)

                if v.lower().startswith('javascript:'):
                    msg = _("JavaScript URI '${value}' not allowed.",
                            mapping={'value': v})
                    raise IllegalHTML(msg)
            ...
            msg = _("Dynamic tag '${tag}' not allowed.",
                    mapping={'tag': tag})
            raise IllegalHTML(msg)
...
def html_marshal(**kw):
    ...
    vars = [ (key + converter, value)
             for key, converter, value in complex_marshal(list(kw.items())) ]
    return tuple(vars)
...
def makeEmail(mtext, context, headers={}):
    ...
    for k, val in list(headers.items()):
