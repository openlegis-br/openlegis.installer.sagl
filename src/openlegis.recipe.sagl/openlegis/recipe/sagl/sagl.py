...
def main(app, parser):
    ...
    if db:
        ...
        logs_create = "CREATE SCHEMA IF NOT EXISTS openlegis_logs DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"

        try:
            db.select_db(mysql_db)
            db.select_db('openlegis_logs')
            created = True
        except:
            cursor.execute(sagl_create)
            cursor.execute(logs_create)
            db.select_db(mysql_db)
            db.select_db('openlegis_logs')
            created = False
        ...
        if not created:
            ...
            logs_dbuser = "CREATE USER 'sagl'@'localhost' IDENTIFIED BY 'sagl'; GRANT ALL ON openlegis_logs.* TO 'sagl'@'localhost';"
            cursor.execute(sagl_dbuser)
            cursor.execute(logs_dbuser)
            ...
            logs_sql = pkg_resources.resource_filename('openlegis.sagl', 'instalacao/openlegis_logs.sql')
            ...
            process = Popen("mysql %s -u%s -p%s -h %s" % ('openlegis_logs', mysql_user, mysql_pass, mysql_host), stdout=PIPE, stdin=PIPE, shell=True)
            process.communicate('source ' + logs_sql)[0]
...
if __name__ == '__main__':
    ...
    parser.add_option("-P", "--mysql-pass", dest="mysql_pass", default="openlegis")
    parser.add_option("-H", "--mysql-host", dest="mysql_host", default="localhost")
    parser.add_option("-D", "--mysql-db", dest="mysql_db", default="openlegis")
